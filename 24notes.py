...
dx = (phi[i+h,j,k]+phi[i-h,j,k]-2*phi[i,j,k])/h**2
dy = (phi[i,j+h,k]+phi[i,j-h,k]-2*phi[i,j,k])/h**2
dz = (phi[i,j,k+h]+phi[i,j,k-h]-2*phi[i,j,k])/h**2
I = mfl.laplacian_operator(phi, dx, dy, dz)
